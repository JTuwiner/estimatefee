...
module.exports = (mempool) => {
	...
	for (const entry of Object.entries(mempool)) {
		const txid = entry[0];
		const t = entry[1];
		const transaction = new Transaction(txid, Math.round(t.fee * 1e8), t.size, t.ancestorsize, Math.round(t.ancestorfees), t.depends);
	...
	// doing a quick flatten:
	let result = []; // transactions

	for (const cluster of clusters) {
		for (const transaction of cluster.transactions) {
			transaction.effectiveFeeRate = cluster.feeRate();
