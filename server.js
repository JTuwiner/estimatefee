...
router.get('/', function*() {
	...
	let estimates = yield Promise.all(range.map(n => bitcoinrpc.estimateFee(n).then(amount =>  ({ n, amount: Math.round(amount * 1e8 / 1000)}) )));
...
router.get('/core-vs-unlimited', function*() {
	yield this.render('core-vs-unlimited', { title: 'Bitcoin Core vs Unlimited' })
});
...
router.get('/n/:n', function*(next) {
	...
	if (!Number.isFinite(n) || n < 2 || n > 1008)
		return yield* next;
...
router.get('/getmempools', function*() {
	...
	const coreProcessed = trimMemPool(coreProcessMempool(mempool));
	const unlimitedProcessed = trimMemPool(unlimitedProcessMempool(mempool));

	this.body = 'window.unlimitedmempool = ' + JSON.stringify(unlimitedProcessed)+ ';\n' +
...
router.get('/mempool-transaction/:txid', function*() {
	...
	const { cluster, ancestors,  descendants } = yield bitcoinrpc.getTransactionCluster(this.params.txid);
	...
	const estimatedBlocks = yield Search(2, 1009, function*(n) {
		...
		console.log('Need: ', nFeeRate, ' for ', n, ' blocks. Which is enough: ', r);
		return r;
	});
